...
trait ModuleResolverImpl extends ModuleResolver {
  this: Syntax with AbstractFile with Errors with Configs with SignatureSerializer =>
  ...
  def resolveDependencies( program: AST, config: Config ): Either[Error, Set[String]] = program match {
    case Program( imports, _, _, _, _, attribute ) =>
      for (
        ...
        resolvedImports <- errorMap( imports.filter( x => ( x.isInstanceOf[UnqualifiedImport] ) || ( x.isInstanceOf[QualifiedImport] ) ), collectImport( config ) ).right
