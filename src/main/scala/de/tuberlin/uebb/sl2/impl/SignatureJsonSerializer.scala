...
trait SignatureJsonSerializer
    extends SignatureSerializer
    with Syntax
    with Errors {
  ...
  //  private def       var2Json(v : Var      ) : JsonExportVar       = v.module + "." + v.ide // why is this commented?
  //  private def    conVar2Json(v : ConVar   ) : JsonExportConVar    = v.module + "." + v.ide // why is this commented?
   ...
   * undocumented
   */
  private def ast2Json( ast: AST ): JsonExportAst = ast match {
    case Program( imports, sigs, _, _, datas, _ ) =>
  ...
  private def json2Sig( jsonSig: ( JsonImportVarName, JsonImportFunctionSig ), location: Location ): ( VarName, FunctionSig ) = {
    ...
    ( json2VarName( fun ), FunctionSig( json2Type( sig ), DefaultModifier, LocationAttribute( location ) ) )
  }
  ...
  private def json2Ctor( jsonCtor: JsonImportConstructorDef, location: Location ): ConstructorDef = {
    ...
    ConstructorDef( json2ConVarName( jsonIde ), json2Types( jsonTypes ), LocationAttribute( location ) )
  }
